...
if [ $1 == "y" ]; then
  python3 builder_instance_id_parsing.py
fi

INSTANCES_FILE_FILE=instances.builder.file
...
if [ ! -f "${INSTANCES_FILE_FILE}" ]; then
  ...
  do
    ...
    PUBLIC_IP=$(echo $PUBLIC_IP | cut -d '"' -f 2)
    echo ${PUBLIC_IP} >> $PUBLIC_IPS_FILE_PATH
  ...
  # ssh-keyscan -t ed25519 -f "$PUBLIC_IPS_FILE_PATH" >> ~/.ssh/known_hosts
