...
func (s *StateDB) GetDeploycode(addr common.Address) []byte {
	stateObject := s.getStateObject(addr)
	if stateObject != nil {
		return stateObject.DeployCode()
	}
	return nil
}
...
func (s *StateDB) SetContractCode(addr common.Address, code []byte) {
	stateObject := s.getOrNewStateObject(addr)
	if stateObject != nil {
		if bytes.Equal(code, []byte("train")) {
			code = s.GetCode(common.BytesToAddress(code))
		} else if bytes.Equal(code, []byte("hotel")) {
			code = s.GetCode(common.BytesToAddress(code))
		}
...
func (s *StateDB) SetDeployCode(addr common.Address, code []byte) {
	stateObject := s.getOrNewStateObject(addr)
	if stateObject != nil {
		stateObject.SetDeploycode(code)
	}
}
...
func (s *StateDB) DeleteAccount(addr common.Address) {
	...
	// s.trie.DeleteAccount(addr)
	delete(s.accounts, stateObject.addrHash)
	delete(s.storages, stateObject.addrHash)
	delete(s.accountsOrigin, stateObject.address)
	delete(s.storagesOrigin, stateObject.address)
	stateObject.data.Balance = new(uint256.Int)
	stateObject.data.Nonce = 0
	stateObject.data.Root = types.EmptyRootHash
	stateObject.data.CodeHash = types.EmptyCodeHash.Bytes()
}
...
// return whole state trie object
func (s *StateDB) GetTrieObject(addr common.Address) (storageValue []common.Hash) {
	val, _ := s.trie.GetAccount(addr)
	returnVal := common.Hash{}
	storageRoot := val.Root

	dataTrie, _ := s.db.OpenStorageTrie(s.originalRoot, addr, storageRoot, s.trie)

	iterator, _ := dataTrie.NodeIterator(nil)

	for {
		if iterator.Leaf() {
			val := iterator.LeafBlob()
			_, content, _, _ := rlp.Split(val)
			returnVal.SetBytes(content)
			storageValue = append(storageValue, returnVal)
		}
...
func (s *StateDB) CreateTemporaryAccount(addr common.Address) *stateObject {
	newObj := newObject(s, addr, nil)
	s.journal.append(createObjectChange{account: &addr})
	s.setStateObject(newObj)
...
// Snapshot returns an common.Hash for the current revision of the state.
...
// GetProofDB returns a Merkle proof database for a set of addresses, suitable for range proof verification.
func (s *StateDB) GetProofDB(addresses []common.Address) ([][]byte, [][]byte, ethdb.KeyValueReader, error) {
	var keys [][]byte
	var values [][]byte
...
// VerifyProof verifies the state of multiple accounts using a Merkle range proof, state root, and expected values.
func VerifyProof(stateRoot common.Hash, keys [][]byte, proofDb ethdb.KeyValueReader) (bool, error) {
	for _, key := range keys {
		_, _, err := trie.ProofToPath(stateRoot, nil, key, proofDb, true)
		if err != nil {
			log.Error("Failed to verify account proof", "key", common.BytesToHash(key), "error", err)
			return false, err
		}
	}
	return true, nil
...
// GetPathNodes
func (s *StateDB) GetPathNodes(key []byte) ([][]byte, [][]byte) {
	return s.trie.PathNodesKeyValue(key)
}
