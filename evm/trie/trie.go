...
func (t *Trie) insert(n node, prefix, key []byte, value node) (bool, node, error) {
	...
	case *shortNode:
		...
		// Track the newly inserted node in the tracer. The node common.Hash
	...
	case nil:
		// New short node is created and track it in the tracer. The node common.Hash
...
// PathNodes returns all nodes from root to leaf for the given key.
// The returned slice contains each node traversed, in order.
func (t *Trie) pathNodes(key []byte) []node {
	var nodes []node
	tn := t.root
	k := keybytesToHex(key)
	pos := 0
	for tn != nil && pos < len(k) {
		nodes = append(nodes, tn)
		switch n := tn.(type) {
		case *shortNode:
			if len(k)-pos < len(n.Key) || !bytes.Equal(n.Key, k[pos:pos+len(n.Key)]) {
				// key not found in trie
				return nodes
			}
			tn = n.Val
		...
		case *fullNode:
			...
			pos++
		case hashNode:
			child, err := t.resolveAndTrack(n, k[:pos])
			if err != nil {
				return nodes
			}
			tn = child
...
func (t *Trie) PathNodesKeyValue(key []byte) ([][]byte, [][]byte) {
	nodes := t.pathNodes(key)
